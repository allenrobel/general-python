...
Summary: Utility classes and methods
...
from os import path # path.exists() path.isfile()
import re
import random, string
import sys

from general_python.general.verify_types import VerifyTypes # Timer()
...
class ErrorMsg(object):
    ...
    Output:
    MyClass exiting with error: this is an error
    ...
    def debug(self,x):
        if type(x) != type(True):
            print('ErrorMsg() exiting. expected True or False for debug(). Got {}'.format(x))
            exit(1)
        self._debug = x

    def error(self, x):
        if self.debug == True:
            print('  {} exiting with error: {}'.format(self.class_name, x))
        else:
            print('{} exiting with error: {}'.format(self.class_name, x))
        exit(1)
...
class Prefix(object):
    ...
    def prefix(self):
        return self.name + '.' + fcaller() + '(' + str(self.version) + '):'
...
def fname():
    ...
    Usage:
       def foo():
           prefix = fname() + ":"
           print prefix + " some comment within foo()"
    '''
...
def sanity_check_file(fn):
    if not file_exists(fn):
        print(f"Exiting. File does not exist: {fn}")
        exit(1)
    if not is_file(fn):
        print(f"Exiting. Is not a file: {fn}")
        exit(1)
...
# move to net_util.py
def up(device,pause=0):
    '''
    Check 10 times if a device is up.  If up, return True, else return false
    By default, check the device immediately.  If pause is passed, sleep pause
    seconds before checking.
    '''
    is_up = False
    cmd = "ping -c 1 {}".format(device)
    count = 1
    if pause != 0:
        print("{} sleeping 10 seconds before checking if {} is up".format(count, device))
        time.sleep(pause)
    while is_up is False and count <= 10:
        count += 1
        for line in pexpect.run(cmd).splitlines():
